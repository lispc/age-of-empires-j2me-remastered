...
随机图（gameMode=0）Easy/Medium/Expert 通用。成绩（2026-09-03 第三批 v30 定型
= v21 行为 + 波次预测器遥测，ailoop `-b` 开 BFS）：Medium 三区间 n=10 合计
**26/29 决胜（89.7%）**——seeds 1000+ 9胜1僵持（1004 退化图）/ 1010+ 9胜1负
（1019 近距 rush，噪声带内）/ 2000+ 8胜2负；Easy n=5 4胜1僵持（同 1004）；
**Expert 1000+ n=10 2胜7负1僵持（决胜 22.2%，与 v21 的 33.3% 同噪声带 ±1-2；
2000+ 泛化抽查 1/10）**。

第三批（波次建模）结论：预测器已落地并验证（hdr[1][55] 滑窗最小二乘 → 发波
跨越点误差≈0，系统偏差全在行军项；摘要行带 `e55/slope/eta`，发波打
`[ai] WAVE launched/cleared`），但架在其上的五个行为变体（v25-v29：抢攻竞速
×3 形态、村民守家参战、村民随军）批量实测全部 ≤ 基线并回滚——**约束在波 1
接战的兵力比（产能硬顶 2 兵营×1/256t + 4 村民）而非时机信息**。行为严格保持
v21。机制新考证（发波电平触发+持续梯队、hdr[55] 幻影塔值、移速装填越大越快、
敌免费资源滴非死代码、难度参数终表）见
...
- Expert（×8 采集 + 每 tick 出兵 + 免费资源滴=敌村民数/1000t）：决胜 22-33% 是
  规则式路线的当前上沿——败局样态为 4-5k 起敌首发波+持续梯队碾过产能上限
  （2 兵营×1/256t）。第三批证伪"时机套利"（预测器够准但抢攻/全民皆兵全输），
  下一突破点研判为波 1 接战微操（塔下风筝/集火优先级/交战纵深）。
