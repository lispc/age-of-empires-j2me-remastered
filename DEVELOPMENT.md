...
### 修复：按 0 全图视图"回不去"（2026-09-01，用户实测报告）
...
当成"再开全图"，形成 **退出即重进的死循环**——按住 `0` 约 0.5s（macOS 连发
延迟）后必现，松手落在哪个视图全看运气。headless 复现：驱动线程直调
`onKeyPress(48)` 并每 50ms 重发（模拟连发），观察 aA 1→8→6→1 循环。
修复（`CanvasPanel`，约 15 行）：记录当前按住的键码，物理键未抬起期间的
同码 keyDown 改投 `keyRepeated`（MIDP 语义，游戏未覆写=忽略，与真机一致）；
窗口失焦时清空按下表防"回来后首按被误判连发"。FIFO `key` 命令直调
`game.onKeyPress`，不经过此路径，dev 工作流不受影响。
验证：连发模拟下 900ms 长按干净退出（aA 稳定 6、dump 画面正常）；
短按进/出、FIFO 回归 `regress.sh` PASS。
