...
1. **#4 剩余 11 个负相位**（当前 9/20）——**2026-09-06 分流复验+两变体证伪**（详见
   aoe/ai/README.md「#4 分流复验」）：中盘竞速带 9 局病理=raider 露营矿线→逃命
   循环→封建 15/15/15 攒不齐。`C4V=swarmcamp`（9/19+1僵）与 `C4V=tower3`
   （恰 9/20）均零净胜不采纳。**仍开放的候选**：WatchTower 提前（v28 科研储备后
   WT 可能终于研究得起了）、敌兵营落成预攒塔料、feudal 攒料优先级（配额禁区慎动）。
...
4. **Easy 原版阈值攻坚**——**2026-09-06 兑现结项**：探针活锁修复（引擎即时取消
   不可达路点→探针游标永不推进=开图瘫痪的存量 bug）+阈值矩阵批测基建
   （`-Daoe.aiAttackThreshold` 覆盖钩子 + `ailoop -T`）。验收（诚实+-b，n=10，
   种子 1000+跳 1004）：**T=50: 10/10、T=100: 10/10、T=150: 9/10、T=200: 9/10
   （38/40）**；Medium 8/10、Expert 1/10 回归带内。基线勘误：早前"0/5"系无
   `-b` 伪基线。残留=seed 1002 型木饥荒图的 T≥100 结构性败局（快败不僵，
   可接受）——与 B.5 专家图木=0 签名同源，若再攻 Expert 可一并看。
5. **Expert 攻坚（2026-09-05 摸底后挂起，未有代码改动）**：
