...
- 【已验证】**帧率与逻辑深度耦合**——"提帧率"不是改一个数字的事（2026-09-01 为
  ...
  - 帧 = 逻辑 tick = 渲染，三者同频。所有游戏常数以 tick 计：移速装填
    （`tickMoveTimer` 每 tick 减 256~1536，**值越大越快**，数值表见 unit-stats.md）、
    远程攻击冷却 15 tick（其余 8）、AI 三周期
...
- 【已验证】桌面鼠标增强（`c.java` "桌面鼠标增强"段，事件链
  ...
  - 屏幕→格子的映射复用 `renderWorld` 的世界渲染遍历做**像素拾取**
    ...
    注意 pending 的清理在 renderWorld 侧完成，mouseTick 只做序号去重后应用）。
...
- 【已验证】FIRE 激活 = `runMenuScript(node, 1)`，脚本操作码 switch（~2150 行）：
  - 操作码 3 = 模式启动：第二个字节是动作码——**65 = Game Mode 自改导航**（按
    ...
    `randomMapDifficulty` = 档位）、**71 = 教学 Mission**（gameMode=16）、**73 = 战役**
...
- 【移植偏差 2026-09-01】Easy 的 `aiAttackThreshold` 原版为 50(配合 20 tick 训练间隔,
  AI 约 2 分钟即凑满军队价值发起 75% 兵力的 all-in,新手无发育窗口,实测被推平)。
  移植版 Easy 提高到 200 给玩家发育窗口;中(60)/高(100)维持原值。防守模式目标
  = 玩家基地[0][8](87.5% 兵力)+ 最近入侵者位置[1][53](12.5%)——经字节码仲裁为
  **原版设计**,非伪影,未改动。触发防御的威胁扫描首轮即武装(0xFFFFFF,setup 时
  写入),不存在模板 0 初值问题。
...
- 【已验证】AI 开关 = `aiEnabled`（旧名 var_boolean_i，字段默认 true）。教学 Mission/
  ...
  冷启动无此问题）。
- 【已验证】AI 大脑 = `tickAi()`（旧名 z()；每帧，更新链 `tickUnits→p→tickAutoEngage→aimProjectiles→
  tickProjectiles→tickAi→tickBuildings→tickMissionScript` 的一环）：
  - **威胁扫描**：每帧比对一个玩家 0 单位与 AI 城镇中心 `hdr[1][8]` 的距离
    （`hdr[1][54]` 高字节 = 扫描指针，低 3 字节 = 当前最近距离²，最近者存 `hdr[1][53]`）。
  - **模式判定**（扫描完成时）：`aiStance = 2`（进攻）当 AI 军队价值 `hdr[1][55]` ≥
    `aiAttackThreshold` 且 ≥ 玩家军队价值 ×1.25；`= 1`（防御/驱赶）当最近入侵者距离²
    ≤ `aiGuardRadiusSq`；否则 0（种田）。军队价值 = Σ存活单位(攻+甲)（死亡时按攻+甲
    扣减，c.java:6690；含村民 1+1=2/个，且**被拆塔的完工加值不回扣**——威慑只增不减）。
...
- 【已验证】三档难度参数（`setupMissionEnv`，gameMode=0 分支；玩家起始资源恒 200/100/100，
  ...
  | `aiFreeResInterval` | MAX | 2500 | 1000 | 敌免费资源滴周期（c.java:8042-8048，**活的**，旧记"死代码"有误）：每次触发给敌三桶各加 `hdr[1][57]`（=敌 t1 村民已建数）；Easy 的 MAX 等于关闭 |
...
- 【实证，非读码】主线玩家代理 20+ 轮实测规律（docs/agent-operations.md §5.3，
  2026-09-03 仲裁时核对与上述代码机制相容，数值本身是经验观察）：任务链滚动难度下，
  m+12 起敌 all-in 规律化——boot 后 10-14k tick、18-26 军、必含 t7×3+t8×2；
  敌出击按接近触发（5-8 格 aggro）而非计时器，阈值随任务浮动；防御模式触发后
  87.5% 兵力直扑玩家 TC、同时有绕后 raid 打经济。我方 boot 工序应按 10k tick
  倒计时排布。

## 任务事件脚本 DSL

- 【已验证】每个任务可带一段**字节码触发器脚本**，装载于 `setupMissionEnv`
  （`missionScript` = res 124/125/126 教学、110..116 战役、128 随机图）。解释器 =
  ...
  （正→负，常用 `-1` = 标记自己实现 fire-once）。4 个帧计数器 `scriptFrameCounters`
...
- 【已验证】**单层迷雾**：`mapTiles` 的 0x8000 = 未探索位，是唯一的视野层。
  主视图 `renderWorld` 第二遍以 `s > 0`（0x8000 未置位）为门，已探索格上的
  敌我单位/建筑/资源全部照常画（无"当前可见/仅记忆"第二层）；小地图
  `stampThumbTile` 对负值（雾）格画黑，单位点刷新走 `(tile & 0x8000) == 0` 门。
  0x4000 = 暗化位（已探明但当前无人看守）：地表贴图走暗化变体（renderWorld
  case 16384 深色 sprite）；且主视图单位渲染跳过"暗化格上非战斗中的敌单位"
  （`c(Graphics,int,int,int)` 开头守卫）——对敌单位构成第二层显隐
  （2026-09-04 wave8 读码补充；第六批"只切明暗"的结论仅限地表）。
- 【已验证】**视野来源**：`p()` 每 tick 对我方全部单位 `revealFogAroundUnit`
  点亮自身 3×3（清 0x8000 与 0x4000）；单位移动步进离开旧格（`tickMoveTimer`）
  与死亡（`removeUnit`）时经 `dimFogAroundUnit` 把旧位置 3×3 置暗化位
  （清 0x8000、置 0x4000）；建筑在 `p()` 中每 tick 轮一座揭圆形：普通建筑半径 3、
  塔半径 6（`void_a`）。探索永久累积、无回雾。开局已探索约 298/4096 格
  （regress golden 开局值）。
- 【已验证】雾格下资源/建筑信息留在低 12 位（0x83xx 资源、0x85xx 建筑）——
  可直接读但属"免侦察地图术"，诚实 AI 不得读。
- 怪癖（引擎原样，勿改）：`renderWorld` 对雾下"行进中（slot[7]&0xFF==1）"的
  单位会顺手 `revealFogAroundUnit` 揭雾并画出（c.java:6252）——GUI 渲染有揭雾副作用；
  `-Daoe.noRender=1` 的批跑无此路径。GUI 与批跑的可见信息集因此有微差。
- 应用：玩家 AI 的"迷雾诚实模式"（`-Daoe.aiFog`，默认开）以"已探索=可见"为
  信息政策，与玩家屏幕逐格一致；实现与消融成绩见
  `docs/research/rulebased-ai-medium-iteration.md`「信息收紧（第六批）」。

## 移动与寻路

- 【已验证】**没有全局寻路**（无 A*/BFS）。单位移动 = `stepUnitMove(玩家, 单位下标)`
  （`c.java` ~5559 行）的单步贪心：
  ...
  4. 被堵时做**局部避障**：按单位朝向 `[i*8+6]` 从方向扇形表 `dirTable`
...
- 推论：单位走直线、遇障局部绕行，凹形障碍会卡住（原地等）——典型 J2ME 时代做法，
  64x64 地图 + 少量单位下够用。
- 每 tick 由主更新循环对每个单位调用（`tickMoveTimer`，按单位速度计数器节流；
  仅 action 0/3 的单位走移动——`tickUnits()` 的 switch 里 case 0 直通 case 3）。
- 【已验证】**单位 action 码**（`slot[7] & 0xF`，aistate 的 `action` 列同源）：
  0=空闲/行军 1=战斗（站定打 `slot[5]` 目标格，远程 15 tick/其余 8 tick 冷却）
  2=采集中 3=携带回存 4=建造/修理。采集/建造/战斗的目标格在 `slot[5]`，
  由抵达钩子 `onUnitArrived(n,n2,tx,ty)` 写入。
- 【移植增强 2026-09-02】**可选 BFS 寻路：`-Daoe.bfsPath=1`**（默认关，关闭时
  `stepUnitMove` 与原行为逐字节一致——新逻辑全部包在 `if (BFS_PATH)` 里，回归 golden 不动）。
  - 只替换"选落点"一步：DDA 直线步进换成"沿缓存的 BFS 路径取下一格"；其后的落点
    三重检查、目标格被占处理（抵达钩子 `onUnitArrived(n,n2,tx,ty)`，采集/接敌靠它）、7 邻格扇形
    回退、占位表更新全部保留。BFS 路径下一格被单位临时占住 → 走原扇形回退；扇形
    全堵 → 原地等（路径位置不动）。
  - 路径存外挂数组（`bfsPath/bfsPathLen/bfsPathPos/bfsPathTarget/bfsPathStuck/
    bfsUnitsBlock`，flat 下标 = 玩家×26+单位，单单位容量 1024 格）：路径是纯缓存，
    可从 slot[2] 重建，不进存档、不动快照格式。
  - 同步策略：到达建议格则推进；被扇形闪避带偏时先**沿原路径归队**（扫描路径找
    当前格续走，避免脱节即重算的风暴），彻底离队才以当前位置重算。
  - 障碍语义：`(t & 0x300) == 0x100`（建筑）与 `== 0x300`（地形资源/虚空）当墙；
    空格与 `== 0x200`（单位占用）当可穿——动态障碍由运行时扇形回退处理。
  - 升级与兜底：沿路径连续 8 次无进展（pos 不前进，含"几格间振荡"活锁）→
    升级为"单位也当墙"重算绕开静态人墙；仍无进展 → 本目标**永久回退原
    DDA+扇形**（基准行为兜底）。目标变更时重置回普通模式重算。
  - 目标格本身不可走（采集资源/攻击建筑）：以目标格的全部可走邻格为种子做多源
    反向洪泛，单位走到目标隔壁即算到位（距离场 dist[cur]==0 时路径长度 0），
    随后交还原 DDA——邻格位置直线一步必落在目标格上，抵达钩子照常触发。
    目标不可达（被建筑围死）或路径超容量 → 同样回退原 DDA+扇形，严格不比原版差。
  - 重算时机：目标变更 / 彻底离队 / 无进展升级；反向 BFS 首次弹出起点即最短，
    提前退出。
  - 确定性：邻格扩张/回溯顺序固定（`BFS_DX/BFS_DY`），无 RNG、无 HashMap/墙钟。
  - 已知边界（2026-09-02 实测）：① 混沌敏感——同一存档点出发,开/关 BFS 的对局
    会发散成不同剧本（单位走位不同 → AI 建筑节奏/布局不同），[result] ticks 只能
    粗比：seed 999 全程局 基准 5524 vs BFS 5487 任务内 tick（≈持平略优）。
    ② AI 把 TC 围成建筑口袋后双方模式都无法交存（已用"冻结档跨模式续跑"实验证明
    基准同样卡死，属既有 AI 布局缺陷，非 BFS 引入）。③ BFS 帧耗时与基准同量级
    （turbo 300s 推进 7.3M~19M tick，与基准 8.7M~15.7M 同区间）；turbo 下
    watchdog 偶发命中 BFS 热代码（帧持续推进,非真卡死）。

## 采集与交存（采集循环定论，2026-09-03 代码仲裁）

- 【已验证】**到达资源格本身才触发采集**：抵达钩子 `onUnitArrived`（c.java:6806，
  唯一调用点 = `stepUnitMove` 步进落点==目标格且该格被占时，c.java:7230）的
  case 768（资源格）：村民（类型 ≤1）置动作 2（采集，`slot[7]=0x6602|kind<<4`，
  0x66=102 tick 一载）、`slot[5]`=资源格。邻格经过/停下不触发——"顺路踩到才采"
  的实测样态即此。
- 【已验证】**正常交存后自动返岗（玩家侧循环成立）**：载满（tickUnits() case 2，
  c.java:6709）→ `slot[2]=nearestDropOff(...)` 选交存点（木取 hdr[8]TC/hdr[9]伐木场较近者；
  金/石取 hdr[8]/[10]/[11] 采矿场最近者）→ 动作 3 回存 → 抵达建筑格 case 256
  （c.java:6820）入账 `hdr[5+kind-1] += hdr[50+kind-1]*multiplier>>8`，随后
  **玩家（n==0）直接 `slot[2]=slot[5]` 写回采集格**（c.java:6840）→ 走回去继续采。
  **"goto/采集绑定一次性、交还后闲置"（手册 r25）与代码矛盾，作废**——绑定后
  是自动循环；观测到的闲置应是下一条的耗尽分支或寻路回退。
- 【已验证】**资源耗尽分支**：每载完成时扣该格余量（`(v&0x7C)>>2`，5 位趟数）；
  扣到 0 → 清格（`&=0xF000`）并**以旧格为中心扫 8 邻域（dirTable 顺序）找同 kind
  资源格**，找到则 `slot[5]` 改指新格、循环继续（c.java:6725-6736）；找不到 →
  村民回到死格，case 768 见 `(v&3)==0` 落空 → 动作 0 **闲置**（手册 r23"树尽退役"
  结案成立，但仅限 8 邻域无同 kind 后继时）。
- 【已验证】**AI 侧不对称**：AI 村民交存后先校验原资源格仍有效才返回
  （c.java:6850-6857；失效时 `findNearbyResource` 的返回值被丢弃——CFR/Vineflower
  一致，非伪影，等价于空操作），真正的补岗在 `tickAi` 每 tick 给"闲置村民"
  （类型<2、动作 0、已到目标）全局重派最近资源格（c.java:7937-7942）。
  玩家侧没有这种全局重派——闲置村民只能靠玩家/玩家 AI 重新指派。
- 【已验证】**交存点选择**（`nearestDropOff`，c.java:8356）：木取 hdr[8]TC 与 hdr[9]伐木场
  较近者；**金/石取 TC + hdr[10]/[11] 双采矿场槽位三者最近**——引擎原生支持
  两个采矿场（placement case 1 先填 [10] 再填 [11]，拆毁清回 -1）。伐木场是
  hdr[9] **单槽**（case 0 无条件覆盖），第二伐木场会劫持全图木交存点。
- 【已验证】**村民修理免费且对完工建筑有效**：村民步进落点==自己建筑格时
  （onUnitArrived case 256，c.java:7026），该建筑 HP<255 即置 action 4（**不查 UC 位**），
  `tickConstruction` 每 tick 50% +1 HP（≈0.5 HP/tick/人，全程零资源消耗）——
  修塔 1 人恰好抵消 1 剑士磨血（4 伤/8t）。UC 建筑不需要村民：`tickBuildings()` 每 tick
  +8 进度 ≈32t 自动完工。
- 【已验证】**生产队列是无类型计数**：`queueUnitTraining` 只做 `rec[+2]+=65536`
  + `hdr[49]++` + `hdr[66+兵种-1]++`；产出时按**建筑+时代**定型（封建后兵营恒产
  剑士）、按产出兵种扣费并 `--hdr[66+产出兵种-1]`（c.java:6040 起）。排队时
  写什么兵种只影响 66+x 计数器——枪兵名额可在封建后买剑士超编（队列语义
  漏洞，RuleBasedAi v54 验证过可用性后按最小 delta 原则未采用）。

## 随机图胜负判定（2026-09-03 代码仲裁，【已验证】）

随机图（gameMode=0）无任务脚本（res#128 = 单字节 127），胜负只有两条代码路径，
**双方不对称**：

- **玩家败**（`startMissionBriefing(0,98,1)`，dialogScriptId=98 briefingVariant=1）：
  ① 玩家 TC（建筑 type 9）被毁即败，**与剩余单位数无关**（建筑移除 `onThingDestroyed()` case 9，
  c.java:7071-7074）——手册 r16"TC 被拆即败"其实是对的，triage r18 的翻案作废；
  ② 最后一个单位死亡**且此刻建筑记录数 hdr[0][4]==0**（单位死亡入口 `removeUnit()`，
  c.java:6656）——只在单位死亡这一刻检查：0 单位但还有建筑站着不判负，之后
  再失去建筑也不会补判（除 TC 走 ①）。
- **玩家胜**（`startMissionBriefing(0,98,0)`，dialogScriptId=98 briefingVariant=0）：**仅"AI TC 被毁"一条**
  （`onThingDestroyed()` case 9，c.java:7076-7077；战役 gameMode=32 且 missionIndex≠0 时该分支
  被跳过，改由任务脚本动作 4/5 判胜负，c.java:8448-8456）。
  **不要求全歼敌单位、不要求清训练队列**——手册 r26"全歼+清队列才弹胜利屏"的
  实证序列里 TC 恰好最后被拆，判定在 TC 倒的一刻就已触发。
- **检测点**：`startMissionBriefing` 对 gameMode=0 写回随机图种子（.nfo 31/32）后
  置 `screenState=12`（结算屏）；`-Daoe.exitOnResult=1` 时打印
  `[result] WIN|LOSS ticks=N` 并退出（c.java:8898-8901，ailoop 的终局信号）。
  弹窗判读位：dialogScriptId=98，`briefingVariant=0`=胜 / `=1`=负（[dlg-parse] 可读）。
- 推论：保 TC 优先级高于保单位（TC 毁=立即判负）；进攻上"拆敌 TC"是唯一
  胜利捷径，扫清敌生产建筑只是战术需要而非判定条件。

## 战斗与索敌（2026-09-03 波 1 微操批读码考证，【已验证】）

- **索敌不是"距离最近"，是"槽序第一个"**：单位/塔的索敌（`acquireTarget`，
  c.java:7418 一带）按对方单位槽顺序扫描，第一个进入判定半径的即锁定——
  集火无法靠走位实现（敌槽序不可控）。deep-dive-2 的"nearestDropOff 选最近目标"系误记。
- **攻击按"格"结算、目标离格白攒装填**：攻击态 `slot[5]`=目标格，`tickUnits()` case 1
  每 tick 校验该格有无敌人，没有则 `slot[7]`（含装填计数）整体清零——近战装填
  8 tick、远程 15 tick，被锁定单位挪一格就废掉攻击者一整次装填（闪避微操的
  引擎依据）。近战接战要 d²≤1（仅正交相邻，斜向不算）。
- **受击反扑改写目标**：被单位命中 → 受害者 `slot[2]`=攻击者位置；被塔命中 →
  `slot[2]`=塔位。"敌军优先打塔"的真身是反扑改写。
- **塔射速**：每塔同时在飞仅 1 发投射物、弹飞 16 tick → ~1 发/17 tick/塔；
  瞄准同样是槽序第一个（塔也无法集火）。
- **索敌/移动节流**：`tickAutoEngage()` 每玩家每 tick 只处理 1 个单位的索敌；我方单位要站在
  目标格上才自动接敌（行军中的部队不自动开打）。
- **群令 `orderMove` 清 `slot[7]`**：周期性群令重发会打断全军攻击装填
  （DPS 暗坑——高频重发群令 = 全军输出归零化）。
- **站桩回血**（2026-09-04 #6 攻关实读）：任务字 0（闲置）且 pos==tgt 且
  `(tickCount&8)!=0` 时 +1 HP（`tickUnits()` case 0，约 0.5 HP/t），全员
  HP 上限 255（出生即满）。原地站不住（pos≠tgt）永远不回——撤退点不可走时
  单位会永久卡在残血态（战役 #6 v6 尸检实锤）。
- **拆建筑与拆人同式**：`resolveAttack` 建筑分支伤害 = `攻<<4 / 建筑甲[33+type]`，
  建筑 HP=255（`tickConstruction` 封顶）。Keep 塔攻 4 → 64/甲 per 17t；
  投石机攻 14 → 对甲 2 单位 112/发、对塔（甲 25）8/发。
- **战役通用"拆敌 TC 即胜"在 missionIndex≠0 被跳过**（`onThingDestroyed`
  case 9，`gameMode==32` 时只对第 0 关生效）——战役关卡的胜利只有脚本链
  一条路，rush 拆 TC 不触发胜利。
...
- **#122** 成本表（150B=50 行×3 字节：单位 0..9 / 建筑 10..25 / 科技 26..49
  （升时代 = 科技 21..23 → 行 47..49），列序 木/金/石——资源只有这三种无食物；
  2026-09-03 全量解包逐行对账，兵种/建筑/科技属性全表见 **unit-stats.md**） **#123** 移动避障方向扇形表 **#124..126** 教学触发脚本
