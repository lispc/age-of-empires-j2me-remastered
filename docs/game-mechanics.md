...
- 【已验证】选关上限只有一处生效点：`activateMenuScreen` 的 menuScreenId-case 11/12
  模板补丁（原版按 `aG+1`/`aj+1` 门控，移植已改为固定 3/7 全解锁）。启动任务时无二次校验。

## AI（敌方玩家，player 1）

- 【已验证】AI 开关 = `var_boolean_i`（字段默认 true）。教学 Mission/随机地图为 true；
  战役除 m4 外 false（静态敌人）。**它只在任务环境装配 `o(int)`（~2945 行）里按模式
  赋值且从不复位**——同一次进程里先玩战役再玩随机图，会继承上一个任务的 AI 开关与
  全部调参（原版引擎的怪癖；冷启动无此问题）。
- 【已验证】AI 大脑 = `z()`（每帧，更新链 `g→p→B→G→J→z→j→F` 的一环）：
  ...
  - **模式判定**（扫描完成时）：`var_int_i = 2`（进攻）当 AI 军队价值 `hdr[1][55]` ≥
    `q` 且 ≥ 玩家军队价值 ×1.25；`= 1`（防御/驱赶）当最近入侵者距离² ≤ `var_int_l`；
    否则 0（种田）。军队价值 = 单位成本之和（训练时累加，见 ~5616 行）。
  - **军队指令**：进攻（模式 2）= 75% 兵力 attack-move 到玩家城镇中心（带 ±1 格抖动）；
    防御（模式 1）= 87.5% 扑玩家城镇中心、12.5% 拦截最近入侵者。村民（类型 <2）空闲时自动
    就近采集（`short_a` 找资源格）。
- 【已验证】AI 经济（同样硬编码，无脚本）：
  - **建造**：写死的建造顺序表 `var_byte_arr_j`（c.java:208，4 阶段：房×2/兵营×1/
    哨塔×2 → 房×3/哨塔×5/铁匠/射箭场/磨坊/兵营 → 房×5/哨塔×5/马厩/射箭场/攻城工坊
    → 房×5/马厩/哨塔×5/射箭场/城堡/兵营×2）。每 `an` tick 跑一步：`void_a()` 从表里
    找第一个未达标建筑设为当前目标 `M`，`boolean_b()` 若买得起就在 AI 基地旁
    `int_h` 螺旋找空位放下（类型计数含在建）。
  - **训练**：每 `C` tick 尝试一次，兵种 = `tickCount % 10` 轮换（村民/长枪/剑士/
    弓兵/侦察/骑兵/冲车/投石车/征服者，受人口 26 与每类上限约束）。
  - **采集加成**：村民交存资源时 AI 产量 × `aM/256`（Easy 2×、Medium ~3.07×、
    Expert 8×）——原版 AI 的资源作弊。
- 【已验证】三档难度参数（`o(int)`，gameMode=0 分支；玩家起始资源恒 200/100/100，
  AI 起始 `hdr[1][5..7]` = 50/15/15（Easy）、50/50/50（Medium）、20/20/20（Expert））：

  | 参数 | Easy | Medium | Expert | 含义 |
  |---|---|---|---|---|
  | `an` | 250 | 150 | 100 | 建造步进周期（tick） |
  | `q` | 50 | 60 | 100 | 进攻所需军队价值 |
  | `C` | 20 | 20 | 1 | 训练尝试周期（tick） |
  | `var_int_l` | 49 | 36 | 25 | 基地警戒半径²（7²/6²/5² 格） |
  | `aw` | MAX | 2500 | 1000 | "定时白送资源"周期——**死代码**：白送量 `hdr[57]` 恒 0 |
  | `aM` | 512 | 786 | 2048 | 采集产量倍率（256=1×） |

- 【已验证】2026-09-01 headless 实测（Easy 随机图，`-Daoe.dev=random:1`）：
  fields dump 命中上表全部参数，37 秒内 `M` 11→12（房→哨塔目标推进）、AI 头数组
  CRC 变化、`aq` 训练计时循环——AI 确实按上述逻辑运转。

## 任务事件脚本 DSL

- 【已验证】每个任务可带一段**字节码触发器脚本**，装载于 `o(int)`（`var_byte_arr_a`
  = res 124/125/126 教学、110..116 战役、128 随机图）。解释器 = `F()`（每帧，更新链
  末位）：顺序扫块，块 = `[条件][动作…][126 结束]`，程序尾 = 127；**块头字节为负 =
  已触发/禁用，跳过**。动作 1 = 解锁某块（负→正），动作 2 = 打已触发标记（正→负，
  常用 `-1` = 标记自己实现 fire-once）。4 个帧计数器 `var_int_arr_a`（动作 3 清零）
  供条件 2 做"经过 N 秒"判定。
- 条件操作码（`int_d(int)`）：1=单位在某矩形 2=计数器×10>阈值 3=画面状态
  4=当前选中==特定单位 5=某槽位单位类型（-1=已死）6=科技旗标==值
  7=玩家头字段 ==/</> 值。
- 动作操作码（`int_d(int,int)`）：0=弹简报页（文本表 = `I`，按模式 71..80）1=解锁块
  2=触发标记 3=清计数器 4=**胜利** 5=**失败** 6=生成单位 7=命令单位/全村民/某类型
  移动 8=写玩家头字段 9=写科技旗标。
- 【已验证】任务数据（`spawnMission`，res 118..120 教学、103..109 战役）是另一段
  数据流：头 13 字节（生成参数 var_int_k/r、双方起始资源、光标位、记录数），其后
  记录：op1 = 放建筑（玩家,类型,x,y,血量）op2 = 生成单位 op3 = 写资源/地形格。
  随机图（gameMode 0）无此数据——地图全由生成器 + `t()` 摆双方城镇中心和村民。

## 游戏模式与关卡

- 【已验证】Game Mode 屏（menuScreenId=1）的循环器是**自改脚本**：FIRE 时跑
  动作 65，按循环器当前值改写自身导航目标——0(Tutorial)→屏 11、1(Campaign)→屏 12、
  2(Random Map)→屏 10。三个模式与 `gameMode` 的真实映射（曾长期搞反，以本条为准，
  证据 = 简报文本内容 + 运行时 fields dump）：
  - **gameMode 16 = 教学 Mission 1..3**（屏 11 "Choose Mission"，动作 71）：
    简报文本 res 78/79/80（"This red box is the cursor…"），任务脚本 124/125/126，
    任务数据 118/119/120。胜利推进 `aG`（0..2，教学关进度）。
  - **gameMode 32 = 战役 Mission 1..7**（屏 12，动作 73）：简报 71..77（剧情连续），
    任务脚本 110..116，任务数据 103..109。胜利推进 `aj`（0..6），写战役高分
    （`nfoHighScores`，得分公式见胜利结算 ~7800 行）。
  - **gameMode 0 = Random Map 遭遇战**（屏 10 "Difficulty"，动作 66）：
    Easy/Medium/Expert 三档 = `var_byte_a` 0/1/2，**无选关、无解锁门**，
    地图由 `AgeOfEmpires.d` 程序化生成，没有任务脚本（res#128 = 单字节 127）。
- 【已验证】`aG`/`aj`（未改名，wave1 曾按注释猜成 random/campaign unlock——
  **aG 实为教学关进度**）：只在胜利结算推进，打包存 `.nfo` 字节 28 = aG<<4|aj。
  选关屏可见项数 = 进度+1（移植已改全解锁，见 `activateMenuScreen` case 11/12）。
- 【已验证】进入关卡流程：选关 → 简报对话框（"Next" = 左软键，桌面按 **F1**）→
  地图。随机地图无简报。
- 【已验证】随机地图生成参数 = `var_int_k`/`r`（传给 `AgeOfEmpires.d` 的种子/规模）；
  教学关/战役来自任务资源头两字节，随机图在 gameMode=0 时读 `.nfo` 字节 31,32
  （默认 8224 = 32,32）——**随机图胜利后会把自己的参数写回 nfo[31,32]**，
  所以下一局默认复用同一张图（想换图需手动改或清档）。
...
- 【已验证】`nfoData`（314 字节），`loadNfo()` 读写 RecordStore `.nfo`
  ...
  - `[28]`：高半字节 `aG`（教学关进度 0..2），低半字节 `aj`（战役进度 0..6）
  - `[29]`：音效开关（0 = 开，对应 `var_boolean_d`）
  - `[30]`：另一开关（`AgeOfEmpires.b.c`，含义未考证）
  - `[31..32]`：随机地图生成参数（高字节 `r`，低字节 `var_int_k`；默认 8224 =
    32,32。随机图胜利时写回 → 下局默认复用同图）
