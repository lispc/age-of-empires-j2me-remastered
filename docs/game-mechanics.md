...
- 【已验证】**单层迷雾**：`mapTiles` 的 0x8000 = 未探索位，是唯一的视野层。
  ...
  0x4000 只切地表贴图明暗（renderWorld case 16384 深色 sprite），不影响显隐。
- 【已验证】**视野来源**：单位每 tick（及移动时经 `l()`）揭自身 3×3（`void_d`）；
  建筑在 `p()` 中每 tick 轮一座揭圆形：普通建筑半径 3、塔半径 6（`void_a`）。
  探索永久累积、无回雾。开局已探索约 298/4096 格（regress golden 开局值）。
- 【已验证】雾格下资源/建筑信息留在低 12 位（0x83xx 资源、0x85xx 建筑）——
  可直接读但属"免侦察地图术"，诚实 AI 不得读。
- 怪癖（引擎原样，勿改）：`renderWorld` 对雾下"行进中（slot[7]&0xFF==1）"的
  单位会顺手 `void_d` 揭雾并画出（c.java:6252）——GUI 渲染有揭雾副作用；
...
- 【已验证】**到达资源格本身才触发采集**：抵达钩子 `c(int,int,int,int)`（c.java:6806，
  唯一调用点 = `boolean_b` 步进落点==目标格且该格被占时，c.java:7230）的
  case 768（资源格）：村民（类型 ≤1）置动作 2（采集，`slot[7]=0x6602|kind<<4`，
  0x66=102 tick 一载）、`slot[5]`=资源格。邻格经过/停下不触发——"顺路踩到才采"
  的实测样态即此。
- 【已验证】**正常交存后自动返岗（玩家侧循环成立）**：载满（g() case 2，
  c.java:6709）→ `slot[2]=int_a(...)` 选交存点（木取 hdr[8]TC/hdr[9]伐木场较近者；
...
- 【已验证】**交存点选择**（`int_a`，c.java:8356）：木取 hdr[8]TC 与 hdr[9]伐木场
...
- 【已验证】**村民修理免费且对完工建筑有效**：村民步进落点==自己建筑格时
  （c() case 256，c.java:7026），该建筑 HP<255 即置 action 4（**不查 UC 位**），
  `tickConstruction` 每 tick 50% +1 HP（≈0.5 HP/tick/人，全程零资源消耗）——
  修塔 1 人恰好抵消 1 剑士磨血（4 伤/8t）。UC 建筑不需要村民：`j()` 每 tick
  +8 进度 ≈32t 自动完工。
...
- **玩家败**（`startMissionBriefing(0,98,1)`，z=98 v=1）：
  ① 玩家 TC（建筑 type 9）被毁即败，**与剩余单位数无关**（建筑移除 `i()` case 9，
  c.java:7071-7074）——手册 r16"TC 被拆即败"其实是对的，triage r18 的翻案作废；
  ② 最后一个单位死亡**且此刻建筑记录数 hdr[0][4]==0**（单位死亡入口 `g()`，
  c.java:6656）——只在单位死亡这一刻检查：0 单位但还有建筑站着不判负，之后
  再失去建筑也不会补判（除 TC 走 ①）。
- **玩家胜**（`startMissionBriefing(0,98,0)`，z=98 v=0）：**仅"AI TC 被毁"一条**
  （`i()` case 9，c.java:7076-7077；战役 gameMode=32 且 missionIndex≠0 时该分支
...
- **检测点**：`startMissionBriefing` 对 gameMode=0 写回随机图种子（.nfo 31/32）后
  ...
  弹窗判读位：z=98，`v=0`=胜 / `v=1`=负（[dlg-parse] 可读）。
- 推论：保 TC 优先级高于保单位（TC 毁=立即判负）；进攻上"拆敌 TC"是唯一
  胜利捷径，扫清敌生产建筑只是战术需要而非判定条件。

## 战斗与索敌（2026-09-03 波 1 微操批读码考证，【已验证】）

- **索敌不是"距离最近"，是"槽序第一个"**：单位/塔的索敌（`boolean_a`，
  c.java:7418 一带）按对方单位槽顺序扫描，第一个进入判定半径的即锁定——
  集火无法靠走位实现（敌槽序不可控）。deep-dive-2 的"int_a 选最近目标"系误记。
- **攻击按"格"结算、目标离格白攒装填**：攻击态 `slot[5]`=目标格，`g()` case 1
...
- **塔射速**：每塔同时在飞仅 1 发投射物、弹飞 16 tick → ~1 发/17 tick/塔；
  瞄准同样是槽序第一个（塔也无法集火）。
- **索敌/移动节流**：`B()` 每玩家每 tick 只处理 1 个单位的索敌；我方单位要站在
