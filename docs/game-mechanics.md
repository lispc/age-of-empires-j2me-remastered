...
- **记录池**：`var_short_arr_arr_b[2][20]`，每玩家 5 条记录 × 4 short，保持紧凑：
  活跃记录占据 `0 .. headers[48]-1`（`[48]` = 活跃投射物数）。发射（攻击 case 12）
  追加在 `[48]<<2` 槽位并 `[48]++`；移除走"尾部交换进空位 + `[48]--`"。
...
- **卡死修复（2026-09-01，战役第一场接战即触发）**：原反编译体把"扫满一圈放弃"的
