...
2. ~~波 1 后产能链韧性~~（第 4 轮已试，判死登记见末节：probeStride=4 镜像批
   13/20 未过线。败因测绘结论：镜像局产能分叉不是"射箭场时机"（45 夜映射
   不同构——村民恒 4 引擎帽、双侧同码同时点放建筑），而是**单资源盲区→收入
   0→军容/军值冻结**（金×3、石×2 = 5/10 种子）；发现提速实锤生效但残余约束
   =4 村民零和下木瓶颈+细流金图+军容帽。复测前提：须与配额/消费侧机制组合）
...
- **资源发现提速方向——probeStride 专职探员跳环（第 4 轮，2026-09-07）**：
  ...
  封不到 14/20**。复测前提：与配额/消费侧机制组合（木地板保护、军容帽解锁
  或 4 村民零和的配额重调）——届时发现侧直接复用 probeStride 即可。旋钮
  默认关留码（含石盲区触发 pullStone）。
