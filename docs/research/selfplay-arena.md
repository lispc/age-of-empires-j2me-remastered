...
1. ~~探图速率~~（第 2 轮已试，判死登记见末节：相关性成立但干预中性——
   侦察卡死修复把敌 TC 发现提前 ~19k tick 仍 10.5/20，早 TC 触发的攻击门
   在对称局负和；攻击门重调前别再回来）
...
6. **攻击触发门的对称局重校**（第 2 轮副产品：现行为"见 TC 即可能攻"是为
   引擎敌波次局调校的；镜像对等局里先攻撞塔环负和——若先把攻击门调对称，
   探图速率收益可能复活）
...
- **探图速率方向——scoutRay 侦察链修复/射线强化（第 2 轮，2026-09-07）**：
  ...
  采纳线，且两变体同分说明全部效果来自修复本身。机制读数：提前找到敌 TC
  会触发为不对等局调校的攻击门（CRUSHED/OVERWHELM），对称镜像局里先攻方
  撞对方塔环负和（45 夜 SPRUSH 同型），探图收益被早攻抵消；对局提速
  ~14%（20688→17836 均 ticks）但胜率不动。**副产品（真 bug 定位，保留在
  ...
  复测前提：攻击门按对称局重新调校后，探图速率才可能兑现为胜率。
