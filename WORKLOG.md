...
### #4 v32-v34：军事护矿 9/20→11/20 + 僵尸投降 TC 存活门（2026-09-06，player-ai）

- **v32 军事护矿**（拉锯桶主死因修复）：v21 尸检病的结构性收口——敌兵在
  塔圈外矿线露营（豁免圈罩不到），而军事只在 TC 12 格内出击，露营者永远
  没人管，村民逃命乒乓、金/石归零。v32：TC 无敌情+塔已立时，军事清
  "村民 8 格内"的站桩/接敌敌兵（只打不动的——768 追 768 追逐僵局，v29）。
  成对批测：+3（相位 9/13/14）/−1（game6），胜局 ticks 还普遍更快。
- **v33 僵尸投降修复**：game12 翻 STALL（烧满 300s 超时）——TC 毁+房屋
  站着+木够时，旧判定 canProduce 恒真永不投降，而 tickCastleRace 见 tc<0
  直接 return 永不训练，两侧互等 35.7M tick。修复：canProduce 加"TC 存活"
  前置（所有关卡通式：0 单位+无 TC=无胜路）。修复后 game12 正常 LOSS
  @6837。同版加：off-corridor 资源搜索枯竭格过滤（趟数==0 跳过）+ 同格
  重派改派次近格（#4 拉锯尾段 16t 重派风暴实锤——虽是败局尾声非死因）。
- **v34 护矿门 milCount≥2**（留 1 兵看家）：game6 没救回（翻负机理是
  护矿期石线渗透断供，不是看家兵数量），同分保留（不变式更稳）。
- **终态 11/20（55%）**：胜相位 3/4/7/9/10/11/13/14/16/19/20。剩余 9 负=
  早死桶（相位 0/112/119 区，首波 1.2k vs 塔 2k 的物理时间差）+game6 型
  拉锯渗透。
- regress 三连绿。

### Easy 阈值通用性复测：主线 0/5 翻案，四档 60-89% 本就通用（2026-09-06，player-ai）

- **新增旋钮**：`-Daoe.aiAtkThreshold=N`（c.java 随机图难度分支后覆盖，
  缺省不动）+ ailoop `-A N` 透传——防线通用性从此可批测。教训：Edit 的
  old_string 用通用 `break; } } }` 模式插错了一次位置（插到 8 千米外
  的循环里），锚点必须带上下文特征。
- **翻案**：主线 b1a7405 的"Easy 阈值 50 首批 0/5（全灭 ~2.5k）"在 merge
  后的树上复现不出来。实测（ailoop -d 1 -a RuleBasedAi -b -A N，n=10，
  种子 1000-1010 跳 1004）：**阈值 50: 8/9、100: 8/9、150: 6/10、
  200: 7/9**。无 -b 复测 2/3+2 STALL 也非 0/5——主线批次疑似无 BFS
  环境产物，存疑（其逐局日志未存证）。
- 阈值响应非单调（150 最弱）：阈值低=敌早出小波=塔防好接；阈值高=憋大波。
  平均 ticks 巨大（35-49k）说明 Easy 收尾慢是常态短板。
- 顺带证实主线断言：seed 1002 的确定性 STALL 在阈值 50 下自然消失
  （WIN@4206，敌早送死）；但 seed 1000 在 50/100 档新出现 STALL
  （收不了尾）。
- future-tasks B.4 已改写为翻案版。regress 三连绿。
