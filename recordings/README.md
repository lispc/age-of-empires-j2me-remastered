...
- m2 经济关  **WIN ticks=6037**（2026-09-06 r70 重录，「先仓后采」铁律——
  金仓/石仓/伐木场前置到位再开采集，34 格长途单趟压到 1-2 格，补员×2，
  零村民死亡）。31 事件 mktrace 逐 tick 一致 + replay-verify WIN 复现
  （TOL=1：回放侧 6038,+1t 为 devBoot 首帧批粒度结构残差,3/3 复现,事件锚
  完全一致）；本地 replay.mp4 625 帧含 win 弹窗。旧长途交付配方录制
  （64077）留档 `recordings/campaign/m2-longhaul/`。
- m3 拆家关  **WIN ticks=2874**（2026-09-06 r69 重录，31×；近战集火直攻
  （retask 风暴禁律三件套：批量指令+沉淀守卫+敌变位才重发）+战间 HEAL
  检查点+2 弓兵帖扫白嫖 4 软楼+哨塔轮换啃）。76 事件 replay-verify 位精确
  PASS + mktrace 事件流 76 行逐 tick 一致；本地 replay.mp4 315 帧。旧"点名
  +自动啃楼"配方录制（89191）留档 `recordings/campaign/m3-chew/`。
...
**宏线七关全清（2026-09-06）**：m0 8576 / m1 5172 / m2 6037 / m3 2874 /
