#!/usr/bin/env python3
"""m4h 制胜驱动 v6.4-h（第 8 轮）= v6.4-e + 残局 N1-N4（r42 定稿，证据 BUGS-m4e/f）。
...
WORK = os.environ.get('M4D_WORK', '/tmp/aoe-camp/m4h')
FIFO = WORK + '/fifo'
LOG = WORK + '/play.log'
...
def main():
    ...
    if len(vslots) > 1:
        cmd(f"retask {vslots[1]} {WOOD_SAFE[1][0]} {WOOD_SAFE[1][1]}", 0.3)
    print(f'retask 扫堂 slot{sweep_slot} → {SWEEP}（N4）；slot1 → WOOD_SAFE '
          f'（boot2：走廊木位退役）', flush=True)
    ...
    while DRY and it < 3000 or (not DRY and time.time() < t_end):
        ...
        # revealFogAroundUnit 只清民兵当前格 3×3，没到位=没探到。改到位才算：
        # 民兵 Chebyshev≤1 贴住矿仓位 (36,40) 或备选 (40,42) 才放行矿仓 build。
        ...
        if not scout_sent and len(mymil) >= 1 and not p1mil and ar >= 1500 \
            ...
            scout_slot = far['slot']
            scout_sent = True
            print(f"ar={ar} 民兵探雾 slot{scout_slot} → (37,41)", flush=True)
        if scout_sent and not scouted:
            near_mine = any(max(abs(u['tile'][0] - 37), abs(u['tile'][1] - 41)) <= 3
                            for u in mymil)
            if not mymil or (not near_mine and not p1mil
                             and ar - last_enemy_seen > 600):
                scout_sent = False    # 探雾民兵阵亡/被抽走：重派
        if not scouted and mymil:
            if any(max(abs(u['tile'][0] - 36), abs(u['tile'][1] - 40)) <= 1
                   or max(abs(u['tile'][0] - 40), abs(u['tile'][1] - 42)) <= 1
                   for u in mymil):
                scouted = True
        ...
        for u in vils:
            ...
            if p1mil or ar - last_wave_birth < 450:
                if j[1] > MINE_YMAX:
                    # 南岗宵禁（r44 boot1/2 尸检定案）：波存活期或波后 450t，木工全员
                    # 离岗。旧几何判据两个洞：①树上采集者不在暴露带→继续采集→满载
                    # 上路撞波（满载回送态 retask 失灵救不回，r35+本轮读码：word7=3
                    # 时 slot[2] 由 FSM 接管）；②西侧"逃向"WOOD_SIDE=自家树格=no-op
                    # 自逃。宵禁把"波期满载"概率归零，代价=波期木收入停摆。
                    side = (PARK_WEST,) if u['tile'][0] < 38 else SAFE
                    s = side[sl % len(side)]
                    fleeing.add(sl)
                    cmd(f"retask {sl} {s[0]} {s[1]}", 0.22)
                    idle_n[f'{sl}'] = 0
                else:
                    # 北岗矿工：旧几何判据（波不涉矿区，勿拽停金/石收入）
                    danger = False
                    if p1mil:
                        d2e = min(dist(m['tile'], u['tile']) for m in p1mil)
                        danger = d2e < 100 or (d2_line(u['tile']) < 81
        ...
        # ---- 闲置卡死重派（仅非避险村民；扫堂期绕开 slot0——到位后本块自然转岗）----
        for u in vils:
            sl = u['slot']
