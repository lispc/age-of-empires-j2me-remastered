    ...
    def tick_world(self):
        ...
        if self.ar % 2700 < 90 and self.p1res[0] >= 10 and self.p1res[1] >= 10:
            ...
            self.p1pool.append(dict(slot=60 + len(self.p1pool), type=4,
                                    tile=[8, 27], hp=255))
            self.p1av += 20
        # all-in 门 → 波次出发（微调 a：按 boot1-3 实测校准——wave1 出生 1314/1634、
        # 波间隔实测 320-811t；敌开局带 2 民兵（av 20），1600 起练兵即达 25 门）
        if self.p1av >= 25 and self.myav < self.p1av * 1.25 and len(self.p1pool) >= 2 \
                and self.ar - getattr(self, 'last_wave_ar', 0) >= 500:
