...
跑法：M4D_DRY=1 python3 mocksim4p.py   （内部 import m4pdrv 并注入 SIM）
...
WOOD_T = {(32, 52), (33, 51)} | set(m4pdrv.WOOD_SAFE)
...
def assert_cursor():
    p = m4pdrv.cursor_path(44, 60, 43, 57)
    assert p == [-1, -1, -4], p
    p = m4pdrv.cursor_path(0, 0, 2, 0)
    assert p == [-2, -4], p
    p = m4pdrv.cursor_path(43, 57, 43, 57)
    assert p == [], p
    p = m4pdrv.cursor_path(45, 58, 43, 57)
    assert p is None, p
    # 偶和格断言：驱动所有候选表必须全偶和
    for name in ['B_CANDS', 'HOUSE_CANDS', 'CAMP_CANDS', 'MILL_CANDS',
                 'BS_CANDS', 'TOWER_CANDS', 'UNIV_CANDS']:
        for (x, y) in getattr(m4pdrv, name):
            assert (x + y) % 2 == 0, f'{name} 含奇和格 ({x},{y})'
    assert sum(m4pdrv.WOOD_BANK) % 2 == 0, '木帽格须偶和'
    # 注：WEST_FLEE/BANK_SCOUT 是行走目标，不吃光标偶和不变量（WOOD_SAFE 本身
    # 就有奇和格 (28,57)）——偶和只约束建造候选。
    assert (m4pdrv.TC[0] + m4pdrv.TC[1]) % 2 == 0
    assert (m4pdrv.FRONT[0] + m4pdrv.FRONT[1]) % 2 == 0
...
class Sim:
    ...
    # ---------- 观测端 ----------
    def snapshot(self):
        # v7.1-p 同构：live aistate 带 target（移动目的地；到位=idle 时
        # target==tile）——驱动 north_walkers 的「真在途」判据依赖它，
        # 缺了 dry 会失真（p0 单位取内部 tgt，未初始化时退化为 tile）。
        us = [dict(slot=u['slot'], p=0, type=u['type'], tile=list(u['tile']),
                   hp=u['hp'], action=u.get('action', 0), sel=False,
                   target=list(u.get('tgt') or u['tile']))
              for u in self.units]
        us += [dict(slot=v['slot'], p=1, type=v['type'], tile=list(v['tile']),
                    hp=v['hp'], action=0, sel=False, target=list(v['tile']))
