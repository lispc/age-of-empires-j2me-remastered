#!/usr/bin/env python3
"""m4o 制胜驱动 v7.1-p（第 16 轮）= v7.0-o + 「G 管道断流」根因修复（r52 读码定案）：
  ①北岗部署死锁（G=0 挂 3000t 的真凶，r15 boot3 实锤+读码双证）：
     north_walkers() 把「卡在 TC 带的北岗在岗者」也数成在途——≥2 个北岗
     村民困在 TC 侧时 n>=2 恒真，闲置重派永不开闸；而唯一的一次 fresh
     retask 又全数撞上波期（north_ok 拒）→ 金/石矿工 3000t 出不了门。
     修：只数「真在途」=aistate target != tile（到位即 target==tile，r15
     终局 aistate 实证）；卡在 TC 的在岗者不再占并发，north_ok 一开窗
     （静默/pinned/450t）即 2 个一批放行。
  ②fleeing 泄漏：单位死亡槽位压缩后 slot 号被复用（r15 终局 aistate：
     slot10=民兵 而 fleeing={10} 恒挂），新单位继承死旗。修：每拍把
     fleeing 剪到当前村民槽集。
  ③交棒 3：塔二座让位封建——tw_max=2 加 age>=1 门（塔 5G 不再抢封建 15G）。
  【证伪】「camp 完工沿金工改派半程点」：无此需求——矿仓完工沿引擎原生
     填 hdr[10]/[11]（c.java:6162 case1），nearestDropOff 金/石取
     min(TC,矿仓*)（c.java:8742 case32/48），金工岗位不变、交存自动改道
     （r15 boot3 camp@2740 完工即生效；矿工不出门才是断流主因）。

v6.9-n（第 14 轮）= v6.8-m + 「G 沙漠」W 收入墙总攻：
...
WORK = os.environ.get('M4D_WORK', '/tmp/aoe-camp/m4p')
FIFO = WORK + '/fifo'
LOG = WORK + '/play.log'
...
def try_research(tag):
    """升时代舞步 v7.0-o（读码+探针双定案，推翻 r51 前全部候选序列）：
    读码（c.java confirmAtCursor 5566 / openBuildingMenu 5356 / 卡片渲染 l 4560）：
    光标在己方完工 TC 格上按 -5(FIRE=keyAction22) → confirmAtCursor →
    openBuildingMenu case 9（门：封建=完工兵营≥1，城堡=Mill+BS≥2；选中态无关）
    → requestStateSwitch(7)=研究卡片态（进入帧 buildActionMenu 置 T=0=确认位，
    selectedTrainProduct=21+age）→ 再按 -5 → 卡片分支 case 6/22/38: T==0 且
    canAfford(0,2,21+age) → y() case2 扣款（封建 15/15/15、城堡 20/20/20，
    res122 实证；本作无食物）→ pendingScreenState=6 回游戏。
    **无 sel、无数字键**——旧舞步（sel 清选中→-5→49→-5）是 14 轮 0 paid 元凶
    （无选中 -5 不在 TC 格位上时 confirmAtCursor 走的是选中/移动分支）。
    付费后 ~32t 完成沿：age+1 + techFlags[15..18]=1（feudal）/[14]=1（castle，
    解锁 University）+ briefing 弹窗（aA=2，-6 清）。
    保留 RESEARCH_LOCK（舞步期间停训练/塔，堵 G 竞态扣款）。
    保留 CURSOR_LOCK 恢复（造偶和 House 5W，完工弹窗把光标吸到偶和格）。"""
...
def _try_research_inner(tag):
    ...
    st2 = fifo_state()
    if st2.get('aA') != 6:
        return 'aA%d_pre' % st2.get('aA')
    if tuple(st2.get('cursor') or (0, 0)) != TC:
        return 'cursor_miss'
    r0 = st2.get('res')
    cmd('key -5', 0.4)                  # ①光标在 TC 上 FIRE → 开研究卡片（aA 6→7）
    st3 = fifo_state()
    if st3.get('aA') == 2:
        cmd('key -6', 0.5)
        return 'popup_mid'
    if st3.get('aA') != 7:
        # aA 纹丝不动=门未过（封建要完工兵营 / 城堡要 Mill+BS≥2）或 am≠aA
        return 'menu_reject'
    cmd('key -5', 0.5)                  # ②卡片确认位 T=0 → y() 扣款
    for _ in range(10):
        time.sleep(0.5 if not DRY else 0)
        st4 = fifo_state()
        if st4.get('aA') == 2:          # 升时代 briefing=完成沿，paid 铁证
            cmd('key -6', 0.5)
            print(f'{tag}: PAID(完成弹窗) {r0} -> {st4.get("res")}', flush=True)
            return 'paid'
        r1 = st4.get('res')
        if r1 and all(r1[i] < r0[i] - 4 for i in range(3)):
            print(f'{tag}: PAID {r0} -> {r1}', flush=True)
            return 'paid'
    st5 = fifo_state()                  # 弹窗可能已被主循环清掉——res 复核
    r1 = st5.get('res')
    if r1 and all(r1[i] < r0[i] - 4 for i in range(3)):
        print(f'{tag}: PAID(尾判) {r0} -> {r1}', flush=True)
        return 'paid'
    return 'nopay'
...
def main():
    ...
    print(f'MODE={"DRY(mocksim)" if DRY else "FIFO(live)"} WORK={WORK} '
          f'VER=v7.1-p WFIX={WFIX} CAMP_FIRST={int(CAMP_FIRST)} '
    ...
    while DRY and it < 3000 or (not DRY and time.time() < t_end):
        ...
        if a.get('aA') == 2:
            ...
            continue
        if a.get('aA') == 7:
            # 研究卡片残留（舞步被弹窗打断等）：-5=确认（可付则付=正果，
            # 不可付则静默退卡，c.java 卡片分支 bl2=false 路径）——探针2 实证
            print(f'warn aA=7 卡片残留 → 补确认', flush=True)
            cmd('key -5', 0.5)
            continue
        if a.get('aA') != 6:
        ...
        if not vils and popu == 0:
            ...
            zomb0 = None
        # v7.1-p：fleeing 泄漏剪枝——死亡槽位压缩后 slot 号被复用（r15 终局
        # aistate 实锤：slot10 已是民兵而 fleeing={10} 恒挂；出坑逻辑只扫
        # 村民，死旗永无沿可清，且新村民复用槽位会继承死旗被误避险）。
        if fleeing:
            fleeing.intersection_update({u['slot'] for u in vils})
        ...
        def north_walkers():
            # 并发上限判据（<2 放行）：只数"真在途"者——aistate target != tile
            # （到位即 target==tile，r15 终局 aistate 实证）。v7.1-p 死锁修复：
            # 旧版把卡在 TC 侧的北岗在岗者也数进来，≥2 个困住时 n>=2 恒真、
            # 闲置重派永不开闸（r15 boot3：金/石矿工 3000t 出不了门=G 断流
            # 主因；fresh retask 全撞波期只是引信）。卡在锚点待推进的不算
            # （dry4 实锤：意图占位互堵死锁），fleeing 不算（避险者不在路上）。
            n = 0
            for v_ in vils:
                ...
                if j_ and j_[1] <= MINE_YMAX and v_['tile'][1] > MINE_YMAX \
                        ...
                        and tuple(v_.get('target') or v_['tile']) \
                        != tuple(v_['tile']):
                    n += 1
        ...
        for u in vils:
            ...
            if sl in fleeing:
                ...
                if j[1] <= MINE_YMAX:
                    # v7.1-p 出坑加人身安全项：旧判据只看敌距**岗位**（TC 围城
                    # 期恒 clear）→ 金工刚被 flag 出门 1-2 拍就被放回，在出生带
                    # 与 d2e<49 圈之间振荡（r16 boot1 实锤：4 金工全程卡 (43-45,
                    # 59-61) 净位移 0）。加「自身距敌全 >49」后：flag→(41,41)
                    # 单向走完→人身+岗位双安全才复工。
                    clear = all(dist(m['tile'], j) > 144 for m in p1mil) \
                        and (not p1mil or min(dist(m['tile'], u['tile'])
                                              for m in p1mil) > 49) \
                        if p1mil else True
                elif mymil:
                    ...
                    if p1mil:
                        ...
                        if pinned:
                            ...
                            danger = d2e < 49 or (d2_line(u['tile']) < 81
                                                  ...
                                                  and not in_birth_band(u['tile']))
                    if danger:
                        # v7.1-p 单向逃逸豁免（与出坑人身项配对）：target 已是
                        # 北向避险/中继点者不再重复夺令——否则 flag→release→
                        # re-flag 每 1-2 拍换向，金工净位移 0 卡死在出生带
                        # （r16 boot1 实锤）。岗位/矿石位真危险不受豁免（target
                        # ==岗位格）。
                        tgt_now = tuple(u.get('target') or u['tile'])
                        if tgt_now in (NORTH_FLEE, NORTH_WAY):
                            continue
        ...
        if not CAMP_FIRST and not camps and not ucon(1) and W >= 15 \
            ...
            if cv:
                print(f'ar={ar} *** 矿仓 {cv} 落成：金/石本地交存 ***', flush=True)
            continue
        # v7.1-p（交棒 3）：塔二座让位封建——age>=1 才开第二座（塔 5G 不再抢
        # 封建 15G；塔一照旧 m>=2 即建）。v6.7-l2：S 盈余时第二座塔。
        tw_max = 2 if (S >= 35 and p0['age'] >= 1) else 1
