...
PLAN = [WOOD_SAFE[0], STONE[0], GOLD[0], GOLD[1], STONE[1], WOOD_SAFE[1],
        GOLD[2], STONE[2], WOOD_SAFE[2]]
...
def main():
    ...
    while DRY and it < 3000 or (not DRY and time.time() < t_end):
        ...
        if not scouted and len(mymil) >= 2 and not p1mil and ar >= 1700 \
                and not camp_any:
        ...
        for u in vils:
            ...
            if idle_poll.get(key) == pos:
                ...
                idle_n[key] = 0
            idle_poll[key] = pos
            if idle_n[key] >= 3 and pos != tuple(j):
                if north_ok(u, j):
                    cmd(f"retask {sl} {j[0]} {j[1]}", 0.22)
                idle_n[key] = 0
