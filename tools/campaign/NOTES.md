...
- **下一轮配方=standoff 白嫖（机制依据链完整，标注推测待 probe）**：胜负只要
  建筑清零，守军无关；塔程²6 < 弓/猫程²16 → **d≥3 站位拆塔零还手**；软建筑
  全无投射物（m5 读码）；守军不被打就永远站桩。中场双塔 (22,50)/(26,28)
  7-9 格无守军=必得；NW 簇站位需逐建筑算「到建筑 ≤16 且到全部守军 ≥36」。
  **先 3 条 probe**：①idle 远程对 d²9-16 建筑自动开火？②猫弹有无 splash
  （有则 NW 簇改弓兵-only）；③威胁分支收紧 d²≤36 副作用。行军走廊避质心
  12 格威胁圈。v4 焦点机保留为被打反击兜底。
- **现行代驱动**=`tools/campaign/m6adrv.py`（v4：集结/逐塔锚点序/焦点滞回/
  粘性站位/卡死检测/healTile——野战交换比是唯一没修的墙）+ `m6a-boot.sh`
  （窄 pkill+base v4 防呆+M6_START_DRV=0 侦察模式）。
