#!/bin/bash
# m6a boot（总攻关 missionIndex 6）：campaign:7 + 新鲜 rmsDir → idx6（progress 0 + 7 - 1）。
# base 落盘 v4 防呆：先观测 aA=2（简报弹窗）→ key -6 推掉 → aA=6 → save →
# 复查 aA 仍 6 且文件 >12KB 才算过（防「简报迟到/弹窗复现」把 base 录坏）。
# 同命令链立刻启动 m6adrv.py；M6_START_DRV=0 则只 boot+录 base（侦察局）。
# 红线：pkill 只用窄 pattern "aoe-camp/m6a"（有并行会话在跑，绝不全局 pkill）。
export JAVA_HOME=/opt/homebrew/opt/openjdk@17
W=/tmp/aoe-camp/m6a
REPO=/Users/zhangzhuo/repos/personal/age-of-empires-j2me-remastered
N=${1:-1}   # boot 序号（drv 日志后缀）
pkill -f "aoe-camp/m6a" 2>/dev/null
sleep 1
...
nohup /opt/homebrew/opt/openjdk@17/bin/java \
  -Daoe.headless=1 -Daoe.dev=campaign:7 -Daoe.tickms=10 -Daoe.debug=1 \
...
if [ "${M6_START_DRV:-1}" = "1" ]; then
  nohup python3 $W/m6adrv.py $W > "$W/drv$N.log" 2>&1 &
  DPID=$!
  disown
