#!/bin/bash
# m6g 主线 boot（总攻关 missionIndex 6）：campaign:7 + 新鲜 rmsDir → idx6。
# base 落盘 v4 防呆（aA=2→key -6→aA=6→save→复查>12KB）。同命令链启动 m6gdrv.py。
# 红线：pkill 只用窄 pattern "aoe-camp/m6g"。
export JAVA_HOME=/opt/homebrew/opt/openjdk@17
W=/tmp/aoe-camp/m6g
REPO=/Users/zhangzhuo/repos/personal/age-of-empires-j2me-remastered
N=${1:-1}
pkill -f "aoe-camp/m6g" 2>/dev/null
sleep 1
...
if [ "${M6_START_DRV:-1}" = "1" ]; then
  nohup python3 $W/m6gdrv.py $W > "$W/drv$N.log" 2>&1 &
  DPID=$!
  disown
