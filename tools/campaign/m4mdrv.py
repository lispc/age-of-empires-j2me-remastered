#!/usr/bin/env python3
"""m4l 制胜驱动 v6.8-m（第 13 轮）= v6.7-l2 + 交棒清单 1/2/4/5 + sim 校准：
...
WORK = os.environ.get('M4D_WORK', '/tmp/aoe-camp/m4m')
FIFO = WORK + '/fifo'
LOG = WORK + '/play.log'
...
def main():
    ...
    print(f'MODE={"DRY(mocksim)" if DRY else "FIFO(live)"} WORK={WORK} '
          f'VER=v6.8-m CAMP_FIRST={int(CAMP_FIRST)} FRONTPLUS={int(FRONTPLUS)} '
          f'RAID_MIN={RAID_MIN} PLAN=2W5G2S B→camp→H→bank', flush=True)
