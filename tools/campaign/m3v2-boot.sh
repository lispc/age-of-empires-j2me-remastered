#!/bin/bash
# m3v2 主线 boot（拆家关 missionIndex 3）：campaign:4 + 新鲜 rmsDir → idx3。
# base 落盘 v4 防呆（aA=2→key -6→aA=6→save→复查>12KB）。同命令链启动 m3v2drv.py。
# 红线：pkill 只用窄 pattern "aoe-camp/<work basename>"。
# 用法: m3v2-boot.sh <boot序号> [workdir]   (workdir 缺省 /tmp/aoe-camp/m3v2;
#       probe 彩排传 /tmp/aoe-camp/m3v2-probe)
export JAVA_HOME=/opt/homebrew/opt/openjdk@17
N=${1:-1}
W=${2:-/tmp/aoe-camp/m3v2}
...
rm -rf "$W/rms" "$W/saves"; rm -f "$W/fifo" "$W/fifo.json" "$W/fifo.aistate.json" \
  "$W/base.aoesave" "$W/mapdump.txt"
mkdir -p "$W/saves" "$W/rms"
mkfifo "$W/fifo"
nohup /opt/homebrew/opt/openjdk@17/bin/java \
  -Dapple.awt.UIElement=true -Daoe.headless=1 -Daoe.dev=campaign:4 -Daoe.tickms=10 -Daoe.debug=1 \
...
if [ "${M3_START_DRV:-1}" = "1" ]; then
  nohup python3 $W/m3v2drv.py $W > "$W/drv$N.log" 2>&1 &
  DPID=$!
  disown
