#!/usr/bin/env python3
"""m4jdrv 离线 dry-run 模拟器：解释驱动命令、演化简化世界，验证状态机分支。
跑法：M4D_DRY=1 python3 mocksim4j.py   （内部 import m4jdrv 并注入 SIM）
...
5. 收入偏高 ~30% 且无距离项：sim 木/金按周期到账，与树/矿→交存点距离无关；
   木银行的真实收益（趟程 16 manh→4-6 manh）在 sim 里不可见，dry 只验证
   「波期西木不被宵禁拉停」这一驱动分支。
6. 村民不死：sim 波不杀村民（真实 earliest 档死于线尾/改派吞货）——木银行的
   保命收益在 dry 里同样不可见。
...
WOOD_T = {(32, 52), (33, 51)} | set(m4jdrv.WOOD_SAFE)
...
def assert_cursor():
    p = m4jdrv.cursor_path(44, 60, 43, 57)
    assert p == [-1, -1, -4], p
    p = m4jdrv.cursor_path(0, 0, 2, 0)
    assert p == [-2, -4], p
    p = m4jdrv.cursor_path(43, 57, 43, 57)
    assert p == [], p
    p = m4jdrv.cursor_path(45, 58, 43, 57)
    assert p is None, p
    # 偶和格断言：驱动所有候选表必须全偶和
    for name in ['B_CANDS', 'HOUSE_CANDS', 'CAMP_CANDS', 'MILL_CANDS',
                 'BS_CANDS', 'TOWER_CANDS', 'UNIV_CANDS']:
        for (x, y) in getattr(m4jdrv, name):
            assert (x + y) % 2 == 0, f'{name} 含奇和格 ({x},{y})'
    assert sum(m4jdrv.WOOD_BANK) % 2 == 0, '木帽格须偶和'
    # 注：WEST_FLEE/BANK_SCOUT 是行走目标，不吃光标偶和不变量（WOOD_SAFE 本身
    # 就有奇和格 (28,57)）——偶和只约束建造候选。
    assert (m4jdrv.TC[0] + m4jdrv.TC[1]) % 2 == 0
    assert (m4jdrv.FRONT[0] + m4jdrv.FRONT[1]) % 2 == 0
...
class Sim:
    ...
    def tick_world(self):
        ...
        for u in self.units:
            if u['type'] <= 1:
                ...
                if d > 2:
                    ...
                    # 矿工全堵 (49,48)，G 全程 0）
                    u['action'] = 2 if (jt in GOLD_T or jt in STONE_T
                                        or jt in WOOD_T) else 0
                    if jt in GOLD_T or jt in STONE_T:
                        trip = 6 if camp_up else 8   # boot1 实测：矿区 ~1.1/100t/vil
                    elif jt in WOOD_T:
                        trip = 3   # boot1 实测：木 ~1.8W/100t/vil（比 r18 m关 慢）
                    else:
        ...
        # 敌练兵
        if self.ar > 1600 and self.ar % 540 < 90 and self.p1res[1] >= 5:
            self.p1res[1] -= 5
        ...
        gap = WAVEGAP or 1000
        earliest = max(2000, WAVE0) if WAVE0 else 2000
        ...
        # 敌行军 → TC（m4f dry2 修正：旧"每轴各走 20"L 形路径会假性贴过矿区
        # （(43,47) 距矿仓 7 格）反复吓跑矿工；真实 BFS 走直线——改直线插值）
        for u in self.p1mil:
