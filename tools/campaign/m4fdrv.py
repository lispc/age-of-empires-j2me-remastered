#!/usr/bin/env python3
"""m4f 制胜驱动 v6.4-f（第 6 轮）= v6.4-e + 残局 N1-N4（r42 定稿，证据 BUGS-m4e/f）。
...
WORK = os.environ.get('M4D_WORK', '/tmp/aoe-camp/m4f')
FIFO = WORK + '/fifo'
LOG = WORK + '/play.log'
    ...
    while DRY and it < 3000 or (not DRY and time.time() < t_end):
        ...
        for u in vils:
            ...
            if p1mil:
                ...
                # 暴露判据（m4f 定案）：距波线 <9 格才算暴露——带状 y 判据会误伤
                # 东翼走廊上的矿工（(41,44) 距线 >10 格但 y∈(42,54)）
                road_exposed = d2_line(u['tile']) < 81
                if d2e < 100 or (road_exposed and ar - last_wave_birth < 450):
                    fleeing.add(sl)
                    # 逃向按侧分流（boot2 教训：西侧村民逃 TC 东南会 45° 横穿
                    # 波线——(36,51)/(35,49) 双尸位）：西半场逃 WOOD_SIDE
                    side = WOOD_SIDE if u['tile'][0] < 38 else SAFE
                    s = side[sl % len(side)]
        ...
        house_emg = len(vils) <= 2 and ar > 1400 and len(mymil) >= 1
