#!/usr/bin/env python3
"""m4fdrv 离线 dry-run 模拟器：解释驱动命令、演化简化世界，验证状态机分支。
跑法：python3 tools/campaign/mocksim4d.py   （内部 import m4fdrv 并注入 SIM）
...
WOOD_T = {(32, 52), (33, 51)} | set(m4fdrv.WOOD_SAFE)
...
def assert_cursor():
    p = m4fdrv.cursor_path(44, 60, 43, 57)
    assert p == [-1, -1, -4], p
    p = m4fdrv.cursor_path(0, 0, 2, 0)
    assert p == [-2, -4], p
    p = m4fdrv.cursor_path(43, 57, 43, 57)
    assert p == [], p
    p = m4fdrv.cursor_path(45, 58, 43, 57)
    assert p is None, p
    # 偶和格断言：驱动所有候选表必须全偶和
    for name in ['B_CANDS', 'HOUSE_CANDS', 'CAMP_CANDS', 'MILL_CANDS',
                 'BS_CANDS', 'TOWER_CANDS', 'UNIV_CANDS']:
        for (x, y) in getattr(m4fdrv, name):
            assert (x + y) % 2 == 0, f'{name} 含奇和格 ({x},{y})'
    assert (m4fdrv.TC[0] + m4fdrv.TC[1]) % 2 == 0
    assert (m4fdrv.FRONT[0] + m4fdrv.FRONT[1]) % 2 == 0
