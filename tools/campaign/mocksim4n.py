...
跑法：M4D_DRY=1 python3 mocksim4n.py   （内部 import m4ndrv 并注入 SIM）
...
WOOD_T = {(32, 52), (33, 51)} | set(m4ndrv.WOOD_SAFE)
...
def assert_cursor():
    p = m4ndrv.cursor_path(44, 60, 43, 57)
    assert p == [-1, -1, -4], p
    p = m4ndrv.cursor_path(0, 0, 2, 0)
    assert p == [-2, -4], p
    p = m4ndrv.cursor_path(43, 57, 43, 57)
    assert p == [], p
    p = m4ndrv.cursor_path(45, 58, 43, 57)
    assert p is None, p
    # 偶和格断言：驱动所有候选表必须全偶和
    for name in ['B_CANDS', 'HOUSE_CANDS', 'CAMP_CANDS', 'MILL_CANDS',
                 'BS_CANDS', 'TOWER_CANDS', 'UNIV_CANDS']:
        for (x, y) in getattr(m4ndrv, name):
            assert (x + y) % 2 == 0, f'{name} 含奇和格 ({x},{y})'
    assert sum(m4ndrv.WOOD_BANK) % 2 == 0, '木帽格须偶和'
    # 注：WEST_FLEE/BANK_SCOUT 是行走目标，不吃光标偶和不变量（WOOD_SAFE 本身
    # 就有奇和格 (28,57)）——偶和只约束建造候选。
    assert (m4ndrv.TC[0] + m4ndrv.TC[1]) % 2 == 0
    assert (m4ndrv.FRONT[0] + m4ndrv.FRONT[1]) % 2 == 0
