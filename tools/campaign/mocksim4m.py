#!/usr/bin/env python3
"""m4mdrv 离线 dry-run 模拟器：解释驱动命令、演化简化世界，验证状态机分支。
跑法：M4D_DRY=1 python3 mocksim4m.py   （内部 import m4mdrv 并注入 SIM）
...
WOOD_T = {(32, 52), (33, 51)} | set(m4mdrv.WOOD_SAFE)
...
def assert_cursor():
    p = m4mdrv.cursor_path(44, 60, 43, 57)
    assert p == [-1, -1, -4], p
    p = m4mdrv.cursor_path(0, 0, 2, 0)
    assert p == [-2, -4], p
    p = m4mdrv.cursor_path(43, 57, 43, 57)
    assert p == [], p
    p = m4mdrv.cursor_path(45, 58, 43, 57)
    assert p is None, p
    # 偶和格断言：驱动所有候选表必须全偶和
    for name in ['B_CANDS', 'HOUSE_CANDS', 'CAMP_CANDS', 'MILL_CANDS',
                 'BS_CANDS', 'TOWER_CANDS', 'UNIV_CANDS']:
        for (x, y) in getattr(m4mdrv, name):
            assert (x + y) % 2 == 0, f'{name} 含奇和格 ({x},{y})'
    assert sum(m4mdrv.WOOD_BANK) % 2 == 0, '木帽格须偶和'
    # 注：WEST_FLEE/BANK_SCOUT 是行走目标，不吃光标偶和不变量（WOOD_SAFE 本身
    # 就有奇和格 (28,57)）——偶和只约束建造候选。
    assert (m4mdrv.TC[0] + m4mdrv.TC[1]) % 2 == 0
    assert (m4mdrv.FRONT[0] + m4mdrv.FRONT[1]) % 2 == 0
    ...
    def tick_world(self):
        ...
        for u in self.units:
            ...
            if ph == 'go':
                ...
                u['wt'] += 1
                if u['kind'] == 'w':
                    # v6.8-m 校准（BUGS-m4l 交棒 7 + boot3 bank 锚点）：
                    #   pre-bank（dropoff=TC，16 manh 腿×2）：trip3+2+2=7it=630ar
                    #   /5W ≈0.79W/100t/vil，2 人 ≈1.59W/100t ✓ live 1.5；
                    #   post-bank（bank 贴树 6 manh 腿×2=1it）：trip1+1+1=3it
                    #   =270ar/5W ≈1.85W/100t/vil ✓ live boot3 实测 263ar/趟
                    #   （bank@1205→barracks@1731，20W/526t/2vil）。
                    trip = 1 if any(b['type'] == 0 for b in self.bld) else 3
                elif u['kind'] == 's':
