#!/usr/bin/env python3
"""m4k 制胜驱动 v6.6-k（第 11 轮）= v6.5-j + 金管道抗断 + RAID_MIN=2（+campfirst 开关）：
...
WORK = os.environ.get('M4D_WORK', '/tmp/aoe-camp/m4k')
FIFO = WORK + '/fifo'
LOG = WORK + '/play.log'
...
def main():
    ...
    print(f'MODE={"DRY(mocksim)" if DRY else "FIFO(live)"} WORK={WORK} '
          f'CAMP_FIRST={int(CAMP_FIRST)} RAID_MIN={RAID_MIN}', flush=True)
    ...
    while DRY and it < 3000 or (not DRY and time.time() < t_end):
        ...
        if G < 5 and ar > 3000 and now > rb_cd and jobs:
            ...
            if len(gj) < 2 and len(wj) >= 3:
                # K1+勘误：挑非回送态者改派（旧代码 retask 时引用陈旧 u——
                # stage_target 用错单位位置，顺手修掉）
                sl = None
                cu = None
        ...
        if CAMP_FIRST and not camps and not ucon(1) and W >= 15 \
            ...
            # K4：矿仓先于木银行（G 早开闸；barracks 自然后推 ~200-400t——
            # dry 校准项，缺省关闭）
        ...
        if bank_slot is not None and bank_scouted and not bank_any \
            ...
            # 建筑格生效，满载回送改道入账后 FSM 自动回树（slot[2]=slot[5]），
            # 波期木收入与改派吞货同根拔除。K5：候选串防占格单点。
        ...
        if not barracks and not ucon(10) and W >= 20 and S >= 10 and now > b_cd \
                and (bank_any or W >= 35 or bank_slot is None):
            # K5：银行「已探明未落地」期间兵营不抢 20W（W<35 时）——boot1 实锤
            # 银行 build FAIL 当拍兵营抽走 W → 银行永欠款 → 退化无银行局。
            # W>=35 = 银行+兵营双款齐备（银行块在本块之前先扣 15）。
