#!/usr/bin/env python3
"""m4k 制胜驱动 v6.7-l（第 12 轮）= v6.6-k + 产能杠杆四件套（结构墙已拆，攻量化墙）：
...
WORK = os.environ.get('M4D_WORK', '/tmp/aoe-camp/m4l')
FIFO = WORK + '/fifo'
LOG = WORK + '/play.log'
...
def main():
    ...
    print(f'MODE={"DRY(mocksim)" if DRY else "FIFO(live)"} WORK={WORK} '
          f'VER=v6.7-l2 CAMP_FIRST={int(CAMP_FIRST)} RAID_MIN={RAID_MIN} '
          f'PLAN=2W5G2S B-first', flush=True)
    ...
    while DRY and it < 3000 or (not DRY and time.time() < t_end):
        ...
        if bank_slot is not None and bank_scouted and not bank_any \
                ...
                and (bool(barracks) or ucon(10) or W >= 35) and now > bank_cd:
        ...
        if len(towers) < tw_max and not ucon(12) and S >= 15 and W >= 20 \
                and G >= 5 and now > t_cd:
        ...
        # L4a 静默/优势蓄金：W/S 已 ≥15 时民兵暂停训练，让 G 攒到 15。m>=1 才蓄
        # （m=0 时村民门依赖 nmil>=1，勿自锁）。
        feudal_wait = (not p0['age']) and bool(barracks) and W >= 15 \
            and S >= 15 and G < 15 and field_ok \
            and ar - last_enemy_seen > 300 and nmil >= 1
        # L2b 塔蓄金：塔未放格且 S/W 已齐时，先把 G 池到 5 让塔落格（boot1：
        # S=15@4316 就位但 G=4<5，塔拖到 4852、完工 ~6100、6300 即殉职——差一拍
        # 全无用）。塔落格后本条件自解（ucon(12)）。
        tower_wait = len(towers) < 1 and not ucon(12) and S >= 15 and W >= 20 \
            and G < 5 and field_ok and ar - last_enemy_seen > 300 and nmil >= 1
