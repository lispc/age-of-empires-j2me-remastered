#!/bin/bash
# m0 主线 boot（拆堡关 missionIndex 0）：campaign:1 + 新鲜 rmsDir → idx0。
# base 落盘 v4 防呆（aA=2→key -6→aA=6→save→复查>12KB）。同命令链启动 m0drv.py。
# 红线：pkill 只用窄 pattern "aoe-camp/m0"。
export JAVA_HOME=/opt/homebrew/opt/openjdk@17
W=/tmp/aoe-camp/m0
REPO=/Users/zhangzhuo/repos/personal/age-of-empires-j2me-remastered
N=${1:-1}
pkill -f "aoe-camp/m0" 2>/dev/null
sleep 1
...
nohup /opt/homebrew/opt/openjdk@17/bin/java \
  -Dapple.awt.UIElement=true -Daoe.headless=1 -Daoe.dev=campaign:1 -Daoe.tickms=10 -Daoe.debug=1 \
...
if [ "${M0_START_DRV:-1}" = "1" ]; then
  nohup python3 $W/m0drv.py $W > "$W/drv$N.log" 2>&1 &
  DPID=$!
  disown
